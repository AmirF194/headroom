...
your responses, not to drive new actions."""
    ...
    async def _execute_native_memory_tool(
        ...
        # The native Anthropic memory tool is another read/write entry point.
        # Apply the same unresolved-project sentinel gate as the custom memory
        # tools before any semantic helper can touch the legacy global backend.
        if request_context is not None:
            _backend, scope, effective_user_id = self._resolve_for_request(
                user_id, request_context
            )
            if self._scope_is_unresolved_project(scope):
                return self._unresolved_project_skip_response(
                    "native", effective_user_id, scope
                )
