...
3. Confirm that a request appears in the wrapper terminal. Savings appear there
   as requests are compressed.

If the health check succeeds but no request appears, reload the VS Code window
and confirm that the extension host can reach the same `127.0.0.1` as Headroom.
